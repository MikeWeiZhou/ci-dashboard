...
/**
 * A_AveragedBuildTimeKpiMapper with simple moving average.
 * 
 * Days with no data will not be plotted (ignored).
 */
export class A_AveragedBuildTimeKpiMapper extends KpiMapper
{
    public Title: string = "Build Time From Queue";
    private _yAxisTitle: string = "Minutes (lower is better)";
    private groupByColumn: any = null;
            ...
            for (let result of jsonArrays[i])
            {
                ...
                if (!traceLines[result[this.groupByColumn]])
                {
                    ...
                    (
                        ...
                        [],                                                 // empty array
                        [],                                                 // empty array
                        (result[this.groupByColumn] == "Overall") ? 2 : 1   // width of trace line
                    );
                }
